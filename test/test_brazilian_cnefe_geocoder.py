#!/usr/bin/env python3
"""
Tests for the optional Brazilian CNEFE geocoder example.
"""
...
EXAMPLE_DIR = os.path.join(REPO_ROOT, "examples", "brazilian_cnefe_geocoder")
sys.path.insert(0, EXAMPLE_DIR)

import import_brazilian_cnefe


class TestBrazilianCnefeGeocoder(unittest.TestCase):
    """Tests the example importer, runtime configuration, and SQL contract."""
    ...
    def test_psql_base_cmd(self):
        with patch.dict(os.environ, {"POSTGRES_USER": "testuser", "POSTGRES_DB": "testdb", "POSTGRES_CONTAINER": "custom_container"}, clear=True):
            cmd = import_brazilian_cnefe.psql_base_cmd()
    ...
    def test_invalid_uf_rejection(self):
        with self.assertRaises(ValueError):
            import_brazilian_cnefe.download_cnefe("XX", "/tmp")

        with self.assertRaises(ValueError):
            import_brazilian_cnefe.import_cnefe_to_postgres("/tmp/fake.zip", "INVALID_UF")
        ...
        try:
            ...
            with patch("subprocess.run") as mock_run:
                ...
                with self.assertRaises(FileNotFoundError):
                    import_brazilian_cnefe.import_cnefe_to_postgres(zip_path, "PA", muni_map={})
        finally:
    ...
    def test_prevent_destructive_limit(self):
        ...
        with patch("subprocess.run") as mock_run:
            ...
            with self.assertRaises(ValueError) as ctx:
                import_brazilian_cnefe.import_cnefe_to_postgres("/tmp/fake.zip", "SP", limit=100)
            self.assertIn("partial --limit replacement was rejected", str(ctx.exception))
    ...
    def test_row_filtering_and_parsing(self):
        ...
        try:
            ...
            with patch("subprocess.run") as mock_run, \
                 ...
                 patch("import_brazilian_cnefe.get_municipality_map", return_value={
                ...
                mock_run.return_value = MagicMock(stdout="0\n")
                import_brazilian_cnefe.import_cnefe_to_postgres(zip_path, "SP")
    ...
    def test_download_cnefe_rejects_invalid_zip_without_content_length(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            with patch("urllib.request.urlopen", return_value=mock_resp):
                with self.assertRaises(IOError) as ctx:
                    import_brazilian_cnefe.download_cnefe("PA", temp_dir)
                    self.assertIn("was not promoted to the cache", str(ctx.exception))
            self.assertFalse(any(name.endswith(".tmp") for name in os.listdir(temp_dir)))
    ...
    def test_cached_zip_skips_full_crc_validation(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(zipfile.ZipFile, "testzip", side_effect=AssertionError("deep validation")), \
                ...
                self.assertEqual(import_brazilian_cnefe.download_cnefe("PA", temp_dir), cache_path)
    ...
    def test_fresh_zip_keeps_full_crc_validation(self):
        ...
        try:
            ...
            with patch.object(zipfile.ZipFile, "testzip", return_value=None) as mock_testzip:
                self.assertTrue(import_brazilian_cnefe.validate_cnefe_zip(zip_path))
                mock_testzip.assert_called_once_with()
                mock_testzip.reset_mock()
                self.assertTrue(import_brazilian_cnefe.validate_cnefe_zip(zip_path, deep=False))
    ...
    def test_download_cnefe_tolerates_cache_removal_before_stat(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            with patch("urllib.request.urlopen", return_value=mock_resp), \
                 patch("import_brazilian_cnefe.os.path.getsize", side_effect=[FileNotFoundError, len(data)]):
                dest = import_brazilian_cnefe.download_cnefe("PA", temp_dir)
    ...
    def test_download_cnefe_uses_unique_temp_paths(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            with patch("urllib.request.urlopen", side_effect=responses), \
                 patch("import_brazilian_cnefe.tempfile.mkstemp", side_effect=capture_mkstemp):
                import_brazilian_cnefe.download_cnefe("PA", temp_dir)
                import_brazilian_cnefe.download_cnefe("RJ", temp_dir)
    ...
    def test_get_target_ufs(self):
        ...
        self.assertEqual(import_brazilian_cnefe.get_target_ufs("SP"), ["SP"])
        self.assertEqual(import_brazilian_cnefe.get_target_ufs("sp, rj, mg"), ["SP", "RJ", "MG"])

        all_ufs = import_brazilian_cnefe.get_target_ufs("BR")
        self.assertEqual(len(all_ufs), 27)
        self.assertIn("SP", all_ufs)
        self.assertIn("AP", all_ufs)
        self.assertIn("DF", all_ufs)

        all_ufs_flag = import_brazilian_cnefe.get_target_ufs("SP", all_flag=True)
    ...
    def test_schema_definition_consistency(self):
        ...
        import_brazilian_cnefe_path = os.path.join(EXAMPLE_DIR, "import_brazilian_cnefe.py")
        with open(import_brazilian_cnefe_path, "r", encoding="utf-8") as f:
    ...
    def test_schema_initialization_is_serialized(self, mock_run):
        ...
        import_brazilian_cnefe.ensure_tables_exist()
    ...
    def test_reverse_geocoding_geography_knn(self):
        ...
        # The importer owns the geography index used by the documented query.
        ...
        self.assertIn("ST_Distance(", docs_content)
        self.assertIn("c.geom::geography <->", docs_content)
        self.assertIn("ST_DWithin(", docs_content)
    ...
    def test_example_compose_security_and_healthcheck(self):
        """The optional example stays scoped, loopback-only, and health-checked."""
        compose_path = os.path.join(EXAMPLE_DIR, "compose.yaml")
    ...
    def test_ci_runs_cnefe_tooling_tests(self):
        ...
        self.assertIn("name: 'Test Brazilian CNEFE geocoder example'", workflow_content)
        self.assertIn("run: python3 test/test_brazilian_cnefe_geocoder.py -q", workflow_content)
        self.assertIn("permissions:\n  contents: read", workflow_content)

    def test_example_image_installs_packaged_extensions(self):
        """The image builds this checkout and init installs packaged extensions."""
        init_sql_path = os.path.join(EXAMPLE_DIR, "initdb", "10_extensions.sql")
        ...
        self.assertIn("make PG_CONFIG=", dockerfile_content)
        self.assertIn("with_llvm=no", dockerfile_content)
    ...
    def test_generate_uuid7(self):
        ...
        u_str = import_brazilian_cnefe.generate_uuid7()
        u = uuid.UUID(u_str)
    ...
    def test_stage_table_name_keeps_uuid_random_bits(self):
        ...
        with patch("import_brazilian_cnefe.generate_uuid7", side_effect=[
            "018f0f00-0000-7000-8000-000000000001",
            "018f0f00-0000-7000-8000-000000000002",
        ]):
            first = import_brazilian_cnefe.stage_table_name("SP")
            second = import_brazilian_cnefe.stage_table_name("SP")
    ...
    def test_isolated_per_uf_staging_table(self, mock_run, mock_popen):
        ...
        try:
            ...
            with patch("import_brazilian_cnefe.ensure_tables_exist"), \
                 patch("import_brazilian_cnefe.stage_table_name", return_value=stage_table) as mock_name:
                with self.assertRaises(subprocess.CalledProcessError):
                    import_brazilian_cnefe.import_cnefe_to_postgres(
                        zip_path,
                        "SP",
    ...
    @patch("subprocess.Popen")
    @patch("subprocess.run")
    def test_import_brazilian_cnefe_zero_count_aborts_safely(self, mock_run, mock_popen):
        """Verify that when 0 valid rows exist, import_brazilian_cnefe_to_postgres aborts without wiping data."""
        mock_proc = MagicMock()
        mock_proc.stdin = io.StringIO()
        mock_proc.wait.return_value = 0
        mock_popen.return_value = mock_proc
        ...
        try:
            ...
            with patch("import_brazilian_cnefe.ensure_tables_exist"):
                with patch("import_brazilian_cnefe.get_municipality_map", return_value={3550308: ("SAO PAULO", "SP")}):
                    with self.assertRaises(RuntimeError) as raised:
                        import_brazilian_cnefe.import_cnefe_to_postgres(zip_path, "SP")
                    self.assertIn("No valid rows were found", str(raised.exception))
    ...
    def test_atomic_swap_guards_partial_import_after_lock(self):
        ...
        try:
            ...
            self.assertIn("Partial --limit replacement rejected", swap_sql)
    ...
    def test_positive_limit_validation(self):
        ...
        for value in ("0", "-1"):
            with self.assertRaises(SystemExit):
                with patch.object(sys, "argv", ["import_brazilian_cnefe.py", "--limit", value]):
                    import_brazilian_cnefe.main()

        for value in (0, -1):
            with self.assertRaises(ValueError):
                import_brazilian_cnefe.import_cnefe_to_postgres("/tmp/fake.zip", "SP", limit=value)

    def test_batch_import_continues_after_a_uf_failure(self):
        """A failed UF is reported after the remaining selected UFs run."""
        with tempfile.TemporaryDirectory() as temp_dir, \
             patch.object(sys, "argv", ["import_brazilian_cnefe.py", "--uf", "SP,RJ,MG", "--dest", temp_dir]), \
             patch("import_brazilian_cnefe.get_municipality_map", return_value={}), \
             patch("import_brazilian_cnefe.download_cnefe", side_effect=["SP.zip", RuntimeError("network down"), "MG.zip"]) as mock_download, \
             patch("import_brazilian_cnefe.import_cnefe_to_postgres", side_effect=[10, 20]) as mock_import, \
             patch("import_brazilian_cnefe.sys.stderr", new_callable=io.StringIO) as stderr, \
             patch("import_brazilian_cnefe.sys.stdout", new_callable=io.StringIO) as stdout:
        ...
        self.assertIn("UF RJ failed: network down", stderr.getvalue())
        self.assertIn("UFs processed successfully: 2", stdout.getvalue())
        self.assertIn("UFs failed: 1", stdout.getvalue())

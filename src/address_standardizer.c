...
PG_MODULE_MAGIC_EXT(.name = "address_standardizer", .version = AS_VERSION);
#else
PG_MODULE_MAGIC;
#endif
...
debug_append_rule_metadata(StringInfo result, const char *matched_rule_string)
{
	...
	if (!matched_rule_string || matched_rule_string[0] == '\0')
	{
				...
				StringInfo rule_out)
{
	...
	if (SPI_processed > 0 && SPI_tuptable)
	{
		...
		matched_rule_string = isnull ? NULL : TextDatumGetCString(rule_value);
...
parse_macro_input(const char *function_name, const char *raw_macro)
{
	 ...
	 * slot. When the parser found a postcode but no explicit state, treat that
	 * location fragment as the state component instead.
	 */
	if ((!parsed_macro->st || parsed_macro->st[0] == '\0') && parsed_macro->zip && parsed_macro->zip[0] != '\0')
	{
		if (parsed_macro->city && parsed_macro->city[0] != '\0')
		{
			parsed_macro->st = parsed_macro->city;
			parsed_macro->city = NULL;
		}
		else if (parsed_macro->address1 && parsed_macro->address1[0] != '\0' && !parsed_macro->num &&
			 !parsed_macro->street && !parsed_macro->street2)
		{
			parsed_macro->st = parsed_macro->address1;
			parsed_macro->address1 = NULL;
		}
	}
